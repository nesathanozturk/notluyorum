...
const NoteCard = () => {
  const renderedNoteCard = notes.map((note) => (
    <div
      key={note.id}
      className={`bg-[${note.cardColor}] py-5 px-6 mb-6 max-w-[25rem] rounded-3xl`}
    >
      <div className="flex justify-between items-center mb-3">
        <h1 className="font-bold text-white">{note.title}</h1>
        <span
          className={`w-16 py-[0.9em] bg-white rounded-xl text-center text-[${note.categoryTextColor}] font-semibold text-xs uppercase tracking-widest`}
        >
          {note.category}
        </span>
      </div>
